...
async def get_notifications(outlet_id: str, db: AsyncSession = Depends(get_db)):
    ...
    return [{"id":str(n.id),"table_id":n.table_id,"customer_name":n.customer_name,
             "type":n.notif_type,"order_preview":n.order_preview or "",
             "order_id": str(getattr(n, "order_id", "")) if getattr(n, "order_id", None) else "",
             "total_amount": float(getattr(n, "total_amount", 0) or 0),
             "order_items": [],
             "session_id":str(n.session_id) if n.session_id else "",
...
async def waiter_action(data: WaiterActionReq, db: AsyncSession = Depends(get_db)):
    ...
    if n.notif_type in ("confirm_session", "customer_arrived") and n.session_id:
