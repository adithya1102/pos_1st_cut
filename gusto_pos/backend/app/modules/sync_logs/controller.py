...
# /sync-logs — which is what makes closing it router-wide safe here, where the
...
#
# Exposed before this change: sync telemetry, readable and writable.
router = APIRouter(prefix="/sync-logs", dependencies=[Depends(get_current_staff)])
